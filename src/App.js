...
import { guardarSesion, getUltimaSesion } from './supabase';
...
export default function App() {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      ejerciciosBase.forEach((ej) => {
        ejerciciosData.push({ nombre: ej.nombre, esExtra: false, series: registros[ej.nombre] || [] });
      });
      extras.forEach((ex) => {
        ejerciciosData.push({ nombre: ex.nombre, esExtra: true, series: ex.series });
      });
  ...
  if (loading) return (
    <div style={{ background: '#0f0f0f', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ color: '#888', fontSize: 14 }}>Cargando tu rutina...</div>
    </div>
  );
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', background: '#0f0f0f', minHeight: '100vh', padding: '20px 16px', color: '#eee' }}>
      <div style={{ maxWidth: 500, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <div style={{ fontSize: 11, color: COLOR, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>{rutinaKey}</div>
          <h1 style={{ fontSize: 22, fontWeight: 800, margin: '0 0 8px', color: '#fff' }}>
            {rutinaKey === 'Rutina 1' ? 'Pierna + Espalda 🦵' : rutinaKey === 'Rutina 2' ? 'Hombros + Bíceps 💪' : 'Pecho + Espalda alta 🏋️'}
          </h1>
          <div style={{ fontSize: 13, color: '#888' }}>{totalHecho}/{totalSeries} series completadas</div>
          <div style={{ height: 6, background: '#222', borderRadius: 99, marginTop: 8, overflow: 'hidden' }}>
          ...
          {ultimaData && <div style={{ marginTop: 8, fontSize: 12, color: '#555' }}>Última sesión: {new Date(ultimaData.fecha).toLocaleDateString('es-ES')} · {ultimaData.nota}/10</div>}
        </div>
