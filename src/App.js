...
const EJERCICIOS_CONOCIDOS = [
‘Arm Extension’, ‘Leg Press’, ‘Low Row’, ‘Leg Extension’, ‘Leg Curl’,
‘Plancha’, ‘Pectoral’, ‘Vertical Traction’, ‘Chest Press’, ‘Incline Chest Press’,
‘Shoulder Press’, ‘Delts Machine’, ‘Curl Scott’,
];

const ORDEN_RUTINAS = [‘Rutina 1’, ‘Rutina 2’, ‘Rutina 3’];

function getSiguienteRutina(ultima) {
if (!ultima) return ‘Rutina 1’;
const idx = ORDEN_RUTINAS.indexOf(ultima);
return ORDEN_RUTINAS[(idx + 1) % ORDEN_RUTINAS.length];
}

function buildRegistros(ejercicios) {
const data = {};
ejercicios.forEach((ej) => {
data[ej.nombre] = ej.series.map((s) => ({
peso: s.pesoRef ? String(s.pesoRef) : ‘’,
reps: String(s.repsRef),
hecho: false,
}));
});
return data;
...
const inp = (disabled) => ({
background: ‘#111’, border: `1px solid ${disabled ? '#1e1e1e' : '#333'}`,
borderRadius: 8, color: disabled ? ‘#444’ : ‘#fff’, fontSize: 14, fontWeight: 600,
textAlign: ‘center’, padding: ‘6px 4px’, width: ‘100%’, boxSizing: ‘border-box’,
});
      ...
      <div style={{ background: '#1a1a1a', borderRadius: 18, padding: 20, marginBottom: 12, border: `1px solid ${COLOR}44` }}>
        ...
        {ultimaData && (
          <div style={{ fontSize: 12, color: '#555' }}>
            Última sesión: {new Date(ultimaData.fecha).toLocaleDateString('es-ES')} · {ultimaData.nota}/10
          </div>
        )}
      </div>

      {/* Botón entrenar */}
      <button onClick={onEntrenar} style={{
...
useEffect(() => {
getHistorial().then((data) => { setSesiones(data || []); setLoading(false); });
}, []);
          ...
          <button onClick={() => setExpandida(isOpen ? null : s.id)} style={{ width: '100%', background: 'none', border: 'none', padding: '14px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ textAlign: 'left' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: '#fff' }}>
                {new Date(s.fecha).toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'short' })}
              </div>
            </div>
            ...
            <div style={{ padding: '0 16px 16px' }}>
              {cardioPrevio.length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  ...
                  {cardioPrevio.map((c, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#aaa', padding: '3px 0' }}>
                      <span>{c.tipo}</span>
                      <span style={{ color: '#666' }}>{c.cantidad} {c.unidad} · {c.intensidad}</span>
                    </div>
                  ))}
                </div>
              )}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
                    {ej.series?.sort((a, b) => a.numero_serie - b.numero_serie).map((sr) => (
                      <div key={sr.id} style={{ background: '#111', borderRadius: 8, padding: '6px 8px', textAlign: 'center' }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 700, color: sr.completada ? '#fff' : '#444' }}>
                          {sr.peso ? `${sr.peso}kg` : '—'} × {sr.reps || '—'}
                        </div>
                      </div>
                    ))}
                  </div>
...
const [screen, setScreen] = useState(‘home’); // home | workout | historial
...
if (data && (!ultimaFecha || data.fecha > ultimaFecha)) {
...
const ejAnterior = ultima.ejercicios.find((e) => e.nombre === ej.nombre);
data[ej.nombre] = ej.series.map((s, i) => {
const serieAnterior = ejAnterior?.series?.find((sr) => sr.numero_serie === i + 1);
return {
peso: serieAnterior?.peso ? String(serieAnterior.peso) : (s.pesoRef ? String(s.pesoRef) : ‘’),
reps: serieAnterior?.reps ? String(serieAnterior.reps) : String(s.repsRef),
hecho: false,
};
});
});
...
const removeSerieBase = (nombre, idx) => setRegistros((p) => ({ …p, [nombre]: p[nombre].filter((*, i) => i !== idx) }));
...
const removeSerieExtra = (nombre, idx) => setExtras((p) => p.map((e) => e.nombre !== nombre ? e : { …e, series: e.series.filter((*, i) => i !== idx) }));
const removeExtra = (nombre) => setExtras((p) => p.filter((e) => e.nombre !== nombre));

const addExtra = () => {
const nombre = newNombre === ‘**custom**’ ? newCustom.trim() : newNombre;
...
ejerciciosBase.forEach((ej) => {
registros[ej.nombre]?.forEach((reg, i) => {
tot++;
if (reg.hecho) comp++;
const ref = ej.series[i] ? (ej.series[i].pesoRef || 0) * ej.series[i].repsRef : 0;
if ((parseFloat(reg.peso) || 0) * (parseInt(reg.reps) || 0) >= ref) mej++;
});
});
...
const n = calcNota();
setNota(n);
setResumen({ reg: registros, ext: extras, cf: cardioFinal, cv: cardio.filter((c) => c.selected) });
};
...
try {
const cardioData = [];
cardio.filter((c) => c.selected).forEach((c) => {
cardioData.push({ momento: ‘previo’, tipo: c.label, cantidad: parseFloat(c.cantidad) || null, unidad: c.unit, intensidad: c.intensidad });
});
if (cardioFinal.activo && cardioFinal.minutos) {
cardioData.push({ momento: ‘final’, tipo: cardioFinal.tipo, cantidad: parseFloat(cardioFinal.minutos), unidad: ‘min’, intensidad: cardioFinal.intensidad });
}
...
```
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 17, fontWeight: 800, color: '#fff' }}>
          {rutinaKey === 'Rutina 1' ? 'Pierna + Espalda 🦵' : rutinaKey === 'Rutina 2' ? 'Hombros + Bíceps 💪' : 'Pecho + Espalda alta 🏋️'}
        </div>
      </div>
    ...
    {ejerciciosBase.map((ej) => (
      <EjCard key={ej.nombre} nombre={ej.nombre} seriesRef={ej.series} rows={registros[ej.nombre] || []} color={COLOR}
        onChange={(i, k, v) => changeBase(ej.nombre, i, k, v)}
        onToggle={(i) => toggleBase(ej.nombre, i)}
        onAddSerie={() => addSerieBase(ej.nombre)}
        onRemoveSerie={(i) => removeSerieBase(ej.nombre, i)} />
    ))}

    {extras.map((ex) => (
      <EjCard key={ex.nombre} nombre={ex.nombre} extra seriesRef={[]} rows={ex.series} color={COLOR}
        onChange={(i, k, v) => changeExtra(ex.nombre, i, k, v)}
        onToggle={(i) => toggleExtra(ex.nombre, i)}
        onAddSerie={() => addSerieExtra(ex.nombre)}
        onRemoveSerie={(i) => removeSerieExtra(ex.nombre, i)}
    ...
    {!resumen && (
      <button onClick={generarResumen} style={{ width: '100%', padding: '16px', background: `linear-gradient(135deg, ${COLOR}, ${COLOR}aa)`, border: 'none', borderRadius: 14, color: '#fff', fontSize: 16, fontWeight: 800, cursor: 'pointer', marginBottom: 20 }}>
        Generar resumen 📊
      </button>
    )}
      ...
      <div style={{ background: '#1a1a1a', borderRadius: 14, padding: 20, border: '1px solid #2a2a2a', marginBottom: 20 }}>
        ...
        {resumen.cv.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            ...
            {resumen.cv.map((c) => (
              <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#ccc', padding: '4px 0', borderBottom: '1px solid #222' }}>
                <span>{c.icon} {c.label}</span><span style={{ color: '#888' }}>{c.cantidad || '—'} {c.unit} · {c.intensidad}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {resumen.cf.activo && resumen.cf.minutos && (
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#ccc' }}>
              <span>🏃 {resumen.cf.tipo}</span><span style={{ color: '#888' }}>{resumen.cf.minutos} min · {resumen.cf.intensidad}</span>
            </div>
          </div>
        )}
        <div style={{ textAlign: 'center', background: '#111', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 48, fontWeight: 900, color: nota >= 8 ? '#22c55e' : nota >= 6 ? COLOR : '#ef4444' }}>
            {nota}<span style={{ fontSize: 20 }}>/10</span>
          </div>
          <div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>
            {nota >= 9 ? '¡Sesión brutal! 🔥' : nota >= 7 ? 'Buen trabajo 💪' : nota >= 5 ? 'Sesión correcta ⚡' : 'Día flojo 😤'}
          </div>
        </div>
        {!guardado ? (
          <button onClick={guardar} disabled={guardando} style={{ width: '100%', padding: '14px', background: guardando ? '#333' : '#22c55e', border: 'none', borderRadius: 12, color: '#fff', fontSize: 15, fontWeight: 800, cursor: guardando ? 'default' : 'pointer' }}>
            {guardando ? 'Guardando...' : '💾 Guardar sesión'}
          </button>
        ) : (
          <div style={{ textAlign: 'center', padding: '14px', background: '#22c55e22', border: '1px solid #22c55e', borderRadius: 12, color: '#22c55e', fontSize: 14, fontWeight: 700 }}>
            ✅ Sesión guardada — <span style={{ cursor: 'pointer', textDecoration: 'underline' }} onClick={() => setScreen('home')}>Volver al inicio</span>
          </div>
        )}
      </div>
    )}
